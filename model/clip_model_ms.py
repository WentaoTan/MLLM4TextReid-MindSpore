...
class AttentionPool2d(nn.Cell):
    def __init__(self, spacial_dim: int, embed_dim: int, num_heads: int, output_dim: int = None, cls_token=None):
        super().__init__()
        # self.positional_embedding = mindspore.Parameter(mindspore.ops.randn(spacial_dim ** 2 + 1, embed_dim) / embed_dim ** 0.5)
        self.positional_embedding = mindspore.Parameter(mindspore.ops.randn((spacial_dim[0] * spacial_dim[1]) + 1, embed_dim)/ embed_dim ** 0.5)
    ...
    def construct(self, x):
        ...
        x = mindspore.ops.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (HW+1)NC

        x = x + self.positional_embedding[:, None, :].to(x.dtype)  # (HW+1)NC

        x, _ = F.multi_head_attention_construct(
            ...
            in_proj_weight=None,
            in_proj_bias=mindspore.ops.cat([self.q_proj.bias, self.k_proj.bias, self.v_proj.bias]),
            bias_k=None,
            bias_v=None,
            add_zero_attn=False,
            dropout_p=0,
...
class LayerNorm(nn.LayerNorm):
    ...
    def construct(self, x):
        orig_type = x.dtype
        ret = super().construct(x.type(mindspore.float32))
        return ret.type(orig_type)


class QuickGELU(nn.Cell):
    def construct(self, x):
        return x * mindspore.ops.sigmoid(1.702 * x)
...
class ResidualCrossAttentionBlock(nn.Cell):
    def __init__(self, d_model: int, n_head: int, attn_mask = None):
        super().__init__()
    ...
    def attention(self, q,k,v):
        self.attn_mask = self.attn_mask.to(dtype=q.dtype) if self.attn_mask is not None else None
...
class Transformer(nn.Cell):
    def __init__(self, width: int, layers: int, heads: int, attn_mask = None, type = ''):
        super().__init__()
        self.width = width
        self.layers = layers
...
class VisionTransformer(nn.Cell):
    def __init__(self, input_resolution: Tuple[int, int], patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.class_embedding = mindspore.Parameter(scale * mindspore.ops.randn(width))
        self.positional_embedding = mindspore.Parameter(scale * mindspore.ops.randn(num_patches + 1, width))
        self.ln_pre = LayerNorm([width])

        self.transformer = Transformer(width, layers, heads,type='visual')

        self.ln_post = LayerNorm([width])
        self.proj = mindspore.Parameter(scale * mindspore.ops.randn(width, output_dim))

        self.part_projector = nn.SequentialCell(*[SimpleMLP(768,768//16,768) for _ in range(4)])
    ...
    def construct(self, x):
        ...
        x = mindspore.ops.cat([self.class_embedding + mindspore.ops.zeros((x.shape[0], 1, x.shape[-1]), dtype=x.dtype), x], 1)  # shape = [*, grid ** 2 + 1, width]
        ...
        # if self.training:   
        #     x = self.transformer(x,stage='s1')
        #     x_part = []
        #     for i in range(len(self.part_projector)):
        #         x_part.append(self.part_projector[i](x))
        #     x_part = mindspore.ops.cat (x_part,dim=1)

        #     x = self.transformer(mindspore.ops.cat ([x,x_part],dim=1),stage='s2')
        # else:
...
class CLIP(nn.Cell):
    def __init__(self,
        ...
        # self.logit_scale = mindspore.Parameter(torch.ones([]) * np.log(1 / 0.07))
        self.part_projector = nn.SequentialCell(*[SimpleMLP(512,512//16,512) for _ in range(4)])
    ...
    def build_attention_mask(self):
        ...
        # mask = mindspore.numpy.empty([self.context_length, self.context_length])
        # mask.fill_(float("-inf"))
        mask = ops.fill(mindspore.float32, (self.context_length, self.context_length), float("-inf"))
        mask = ops.triu(mask)
    ...
    def encode_text(self, text):
        ...
        x = x[mindspore.ops.arange(0, x.shape[0]), mindspore.ops.argmax(text,dim=-1)] @ self.text_projection
...
def resize_pos_embed(posemb, posemb_new, hight, width):
    ...
    posemb = mindspore.ops.cat ([posemb_token, posemb_grid], dim=1)
...
def build_CLIP_from_openai_pretrained(name: str, image_size: Union[int, Tuple[int, int]], stride_size: int, jit: bool = False, download_root: str = None):
    ...
    try:
        # loading JIT archive
        model = CLIP(**{'embed_dim': 512, 'image_resolution': (384, 128), 'vision_layers': 12, 
                        'vision_width': 768, 'vision_patch_size': 16, 'context_length': 77, 
                        'vocab_size': 49408, 'transformer_width': 512, 'transformer_heads': 8, 
                        'transformer_layers': 12, 'stride_size': 16}
    )
        param_dict = mindspore.load_checkpoint("ms_clip.ckpt")
        ...
        if jit:
            ...
            jit = False
        state_dict = mindspore.load(model_path, map_location="cpu")
    ...
    model_cfg = {'embed_dim': 512, 'image_resolution': (384, 128), 'vision_layers': 12, 'vision_width': 768, 'vision_patch_size': 16, 'context_length': 77, 'vocab_size': 49408, 'transformer_width': 512, 'transformer_heads': 8, 'transformer_layers': 12, 'stride_size': 16}
...
class SimpleMLP(nn.Cell):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.hidden1 = nn.Dense(input_size, hidden_size)
