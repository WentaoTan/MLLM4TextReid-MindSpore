...
from datasets.mals import MALS_PEDES
...
__factory = {'CUHK-PEDES': CUHKPEDES, 'ICFG-PEDES': ICFGPEDES, 'RSTPReid': RSTPReid,
             'GPTDIFFBLIP_PEDES':GPTDIFFBLIP_PEDES, 'LuPerson_PEDES':LuPerson_PEDES,
             'MALS_PEDES':MALS_PEDES, }
...
def build_mix_loader(args, dataset):
    logger = logging.getLogger("IRRA.dataset")
    ...
    train_transforms = build_transforms(img_size=args.img_size,
                                            aug=args.img_aug,
                                            is_train=True)
    train_set = ImageTextMLMDataset(dataset,
                            train_transforms,
                            text_length=args.text_length)
    train_loader = DataLoader(train_set,
                                batch_size=args.batch_size,
