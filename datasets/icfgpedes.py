        ...
        if training:
            ...
            for anno in annos:
                ...
                for caption in captions:
                    ...
                    inblip_text += " "+caption
                image_id += 1
            # inblip_text = remove_punctuation_and_spaces(inblip_text)
            # a = inblip_text.lower().split()
            # print(len(a))
            # print(len(set(a)))
            # import json
            # with open('./caption/icfg_words.json','w') as file:
            #     json.dump(list(set(a)),file)
            # import IPython
            # IPython.embed()
            # with open('/data4/guowei/wentao/IRRA-main/caption/Lup_qwen.json', 'r') as json_file:
            #     inblip = json.load(json_file)
            #     text = [v[0] for v in inblip.values()]
            #     inblip_text = ' '.join(text)
            # text = []
            # with open('/dev/shm/SYNTH-PEDES/synthpedes-dataset.json', 'r') as json_file:
            #     inblip = json.load(json_file)
            #     for item in inblip:
            #         for t in item['captions']:
            #             text.append(t)
            #         text.append(item['prompt_caption'][0])
            #     # text = [v[0] for v in inblip]
            # #     inblip_text = ' '.join(text)
            # inblip_text = remove_punctuation_and_spaces(inblip_text)
            # b = inblip_text.split(' ')
            # print(len(b))
